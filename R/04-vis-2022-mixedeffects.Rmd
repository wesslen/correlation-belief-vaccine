...
subtitle: Vis 2022 Belief Correlation
...
m = lmer(diffBeliefAbs ~ vis_condition * preBeliefDistance +  (1|usertoken),df)
# Uncertainty Difference
m1 = lmer(diffUncertainty ~ vis_condition * preBeliefDistance + (1|usertoken),df)
```
...
bm <- brms::brm(diffBeliefAbs ~ visTreatment * preBeliefDistance + visTreatment * sampleUncertainty +  sampleUncertainty * preBeliefDistance + (1|usertoken), data = df)

save(bm, file = "../models/2022/fit_bm.rda")
```

```{r}
load("../models/2022/fit_bm.rda")
```
...
```{r}
p_mb2 <- tibble::as.tibble(bm2_u$prior)

select(p_mb2, prior:group)
```
